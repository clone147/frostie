...
import { readFile as readFile3, rename, rm as rm3, writeFile as writeFile4 } from "node:fs/promises";
import { createServer } from "node:http";
import { dirname as dirname5, extname, join as join6, normalize, resolve as resolve7 } from "node:path";
import { fileURLToPath as fileURLToPath3 } from "node:url";
function studioPaths(configPath) {
  const configDir = dirname5(resolve7(configPath));
  const outDir = join6(configDir, "out");
  return {
    configPath: resolve7(configPath),
    configDir,
    outDir,
    webDir: join6(outDir, "web"),
    designFile: join6(configDir, "goldie.design.json"),
    exportZip: join6(outDir, "export.zip")
  };
}
...
function designHandler({ paths }) {
  return (req, res) => {
    if (req.method === "GET") {
      readFile3(paths.designFile, "utf8").then((json) => {
    ...
    readBody(req).then(async (body) => {
      ...
      try {
        ...
        await writeFile4(tmp, `${JSON.stringify(design, null, 2)}
`);
...
function exportHandler({ paths, cli }) {
  let busy = false;
  return (sub) => (req, res) => {
    if (req.method === "GET" && sub === "/download") {
      if (!existsSync3(paths.exportZip)) {
        res.statusCode = 404;
    ...
    readBody(req).then(async (body) => {
      ...
      const flags2 = [];
      if (opts.background)
        flags2.push("--background", opts.background);
      if (opts.frame)
        flags2.push("--frame", opts.frame);
      if (opts.font)
        flags2.push("--font", opts.font);
      if (opts.template)
        flags2.push("--template", opts.template);
      if (opts.layout)
        flags2.push("--layout", opts.layout);
      if (opts.screenOnly)
        flags2.push("--screen-only");
      res.writeHead(200, {
...
`);
          await stream(bin, [...prefix, command, ...flags2], paths.configDir, res, {
            GOLDIE_CONFIG: paths.configPath
          });
        }
        res.write(`$ zip screenshots + previews
`);
        await rm3(paths.exportZip, { force: true });
        await stream("zip", ["-r", "-q", paths.exportZip, "screenshots", "previews"], paths.outDir, res);
        res.write(`[done]
`);
      } catch (err) {
...
function stream(cmd, args, cwd, res, env = {}) {
  return new Promise((done, fail) => {
    const child = spawn2(cmd, args, { cwd, env: { ...process.env, ...env } });
...
function fileIn(root, urlPath) {
  const rel = normalize(decodeURIComponent(urlPath)).replace(/^(\.\.[/\\])+/, "");
  const file = join6(root, rel);
  if (!file.startsWith(root))
    return null;
  return existsSync3(file) && statSync(file).isFile() ? file : null;
}
var STUDIO_DIST = resolve7(dirname5(fileURLToPath3(import.meta.url)), "..", "studio", "dist");
function serveStudio(api, port = 4321) {
  if (!existsSync3(join6(STUDIO_DIST, "index.html"))) {
    throw new Error(`No studio build at ${STUDIO_DIST}. In a source checkout run: bun run studio:build`);
  }
  ...
  const server = createServer((req, res) => {
    ...
    if (path === "/api/design")
      return design(req, res);
    if (path.startsWith("/api/export"))
      return exp(path.slice("/api/export".length))(req, res);
    ...
    sendFile(req, res, join6(STUDIO_DIST, "index.html"));
  });
  return new Promise((done, fail) => {
...
async function main() {
  ...
  const cfg = await loadConfig(opt("config") ? resolve8(process.cwd(), opt("config")) : undefined);
  const font = opt("font");
  applyDesign(cfg, {
  ...
  validateLayouts(cfg);
  const devices = opt("device") ? [opt("device")] : cfg.devices;
  ...
  switch (command) {
    ...
    case "studio": {
      ...
      const url = await serveStudio({
        ...
        cli: [process.execPath, fileURLToPath4(import.meta.url)]
...
function packageVersion() {
  ...
  return JSON.parse(readFileSync2(pkg, "utf8")).version;
}
async function runCapture(cfg, devices) {
  for (const d of devices) {
    const udid = await resolveUdid(d);
    try {
      await capture(cfg, d);
    } finally {
...
async function verifyAll(cfg, devices, locales) {
  let ok = true;
  for (const d of devices)
    for (const l of locales)
      ok = await verify(cfg, d, l) && ok;
  return ok;
}
...
//# debugId=AB47551E2E0A8A7864756E2164756E21
